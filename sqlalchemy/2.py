...
print query.filter(Student.id == 2).first().name
print query.get(2).name # 以主键获取，等效于上句
print query.filter(text('id = 2')).first().name # 支持字符串


query2 = session.query(Student.name)
print query2.all() # 每行是个元组
print query2.limit(1).all() # 最多返回 1 条记录
print query2.offset(1).all() # 从第 2 条记录开始返回
print query2.order_by(Student.name).all()
print query2.order_by('name').all()
print query2.order_by(Student.name.desc()).all()
print query2.order_by(text('name desc')).all()
print session.query(Student.id).order_by(Student.name.desc(), Student.id).all()#删除数据
print query2.filter(Student.id == 1).scalar() # 如果有记录，返回第一条记录的第一个元素
print session.query('id').select_from(Student).filter(text('id = 1')).scalar()
print query2.filter(Student.id > 1, Student.name == 'wangxing').scalar() # and


query3 = query2.filter(Student.id > 1) # 多次拼接的 filter 也是 and
query3 = query3.filter(Student.name == 'wangxing')
print query3.scalar()
print query2.filter(or_(Student.id == 1, Student.id == 2)).all() # or
print query2.filter(Student.id.in_((1, 2))).all() # in

query4 = session.query(Student.id)
print query4.filter(Student.name == None).scalar()
print query4.filter(text('name is null')).scalar()
print query4.filter(not_(Student.name == None)).all() # not
print query4.filter(Student.name != None).all()
